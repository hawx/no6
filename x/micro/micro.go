...
func (s *Store) Find(predicates []string, qs ...no6.SubjectMatcher) (map[string]any, bool) {
...
func (s *Store) FindAll(predicates []string, qs ...no6.SubjectMatcher) []map[string]any {
	subjects := s.inner.QuerySubjects(qs...)
	if len(subjects) == 0 {
		return nil
	}
